...
namespace config
{
	...
	void create(const std::string& config_name)
	{
		...
		auto& rage = json_obj[CXOR("rage")];
		{
			...
			save_int(rage, CXOR("target_selection"), default_config.rage.target_selection);
	...
	void save(const std::string& config_name)
	{
		if (LUA_SYSTEM) 
		{
			LUA_SYSTEM->trigger_event(CXOR("pre_config_save"));
		}
		...
		if (LUA_SYSTEM)
		{
			LUA_SYSTEM->trigger_event(CXOR("post_config_save"));
		}
	}
	...
	void load(const std::string& config_name)
	{
		if (LUA_SYSTEM)
		{
			LUA_SYSTEM->trigger_event(CXOR("pre_config_load"));
		}
		...
		auto& rage = json_obj[CXOR("rage")];
		{
			...
			load_int(rage, CXOR("target_selection"), default_config.rage.target_selection);
		...
		if (LUA_SYSTEM)
		{
			LUA_SYSTEM->trigger_event(CXOR("post_config_load"));
		}
	}
}