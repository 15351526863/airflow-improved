...
const char* vis_chams_type[] =
{
	...
	xor_strs::vis_chams_wireframe.c_str(),
};
		...
		case 2:
		{
			...
			ImGui::SetColumnOffset(1, 270);
			{
				begin_child(CXOR("Main"))
				{
					...
					if (g_cfg.antihit.jitter_mode > 0)
					{
						...
						if (g_cfg.antihit.jitter_mode <= 2)
							slider_int(CXOR("Delay##jitter"), &g_cfg.antihit.jitter_delay, 0, 16, CXOR("%d ticks"));
					}
			...
			ImGui::NextColumn();
			{
				...
				//	checkbox(CXOR("Jitter move"), &g_cfg.antihit.jitter_move);
					...
					checkbox(CXOR("Randomize delay"), &g_cfg.antihit.random_delay);
				}
				end_child;
#endif
