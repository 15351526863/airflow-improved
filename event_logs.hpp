...
public:
	INLINE void push_message(const std::string& message,
		const c_color& color = { 255, 255, 255, 255 },
		bool              debug = false)
	{
...
#ifdef _DEBUG
inline std::unique_ptr<c_event_logs> EVENT_LOGS{ std::make_unique<c_event_logs>() };
#else
