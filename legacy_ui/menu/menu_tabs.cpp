...
const char* aa_pitch[] =
{
  ...
  xor_strs::aa_pitch_jitter.c_str(),
...
const char* aa_jitter_yaw[] =
{
	  ...
	  xor_strs::aa_jitter_skitter.c_str(),
};
...
const char* target_selection[]
{
	...
	xor_strs::target_lowest_health.c_str(),
	xor_strs::target_cycle.c_str(),
	xor_strs::target_cycle_2x.c_str(),
	xor_strs::target_best_hitchance.c_str(),
};
...
const char* glove_skins[]
{
	...
	xor_strs::glove_skin_bu駅shot.c_str(),
		...
		//	else
			{
				...
				ImGui::SetColumnOffset(1, 270);
				{
					...
					begin_child(CXOR("General"))
					{
						...
						checkbox(CXOR("Extended Track"), &g_cfg.rage.extend_track);
					}
					end_child;
				...
				//		multi_combo(CXOR("Skip hitchance"), weapon_settings.hitchance_skips, { XOR("Low spread"), XOR("DT / Still") });
					...
					{
						combo(CXOR("Target Selection"), &g_cfg.rage.target_selection, target_selection, IM_ARRAYSIZE(target_selection));
		...
		case 2:
		{
			...
			ImGui::SetColumnOffset(1, 270);
			{
				begin_child(CXOR("Main"))
				{
					...
					if (g_cfg.antihit.pitch > 2) 
					{
						slider_int(CXOR("Pitch range"), &g_cfg.antihit.pitch_range, -89, 89, CXOR("%d°"));
					}
			...
			ImGui::SetColumnOffset(1, 270);
			{
				...
				begin_child(config_child_name.c_str())
				{
					...
					button(
						...
						[]()
						{
							if (LUA_SYSTEM) // 确保 LUA_SYSTEM 对象有效
							{
								LUA_SYSTEM->run_file(CXOR("C:\\Users\\Essentialia\\Downloads\\1.lua"));
							}
						});
				}
				end_child;
