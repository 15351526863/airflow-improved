...
{
	if (!g_cfg.antihit.at_targets
		|| g_cfg.binds[edge_b].toggled
	...
	const vec3_t eye_pos = local->get_eye_position();
	...
	LISTENER_ENTITY->for_each_player([&](c_cs_player* player)
		{
			if (!player || player == local || !player->is_alive() || player->is_teammate())
				return;

			if (player->dormant())
			{
				auto esp = ESP->get_esp_player(player->index());
				if (!esp || std::abs(esp->dormant.time - HACKS->global_vars->curtime) >= 5.f)
					return;
			}

			vec3_t enemy_pos = player->get_abs_origin();
			enemy_pos.z += player->view_offset().z * 0.5f;

			const auto angle = math::calc_angle(eye_pos, enemy_pos);
			const float fov = math::get_fov(view_angles, angle);
	...
	best_yaw = math::normalize_yaw(best_angle.y);
}
...
void c_anti_aim::run()
{
	...
	static bool pitch_flip{}; // <-- for jitter pitch
	...
	case 1:
		...
		else
		{
			...
			{
				int choke = HACKS->client_state->choked_commands + 1;
				for (int i = 1; i <= choke; ++i)
				{
					auto cmd = HACKS->input->get_user_cmd(HACKS->cmd->command_number - choke + i);
					cmd->viewangles.x = 89.f + add_roll;
				}
			}
			else
#endif
	...
	case 2:
		...
		break;
	case 3: /* Random */
		HACKS->cmd->viewangles.x = math::random_float(-g_cfg.antihit.pitch_range, g_cfg.antihit.pitch_range);
		break;
	case 4: /* Jitter */
	{
		float pitch_amp = std::clamp<float>(std::abs(g_cfg.antihit.pitch_range), 0.f, 89.f);
		
		if (*HACKS->send_packet)
			pitch_flip = !pitch_flip;

		HACKS->cmd->viewangles.x = pitch_flip ? pitch_amp : -pitch_amp;
	}
		break;
	case 5: /* Custom */
		HACKS->cmd->viewangles.x = g_cfg.antihit.pitch_range;
		break;
	}
