...
void c_anti_aim::run()
{
	...
	if (*HACKS->send_packet)
	{
		...
		static int jitter_ticks{};
		static int jitter_goal{};
		if (g_cfg.antihit.jitter_mode <= 2)
		{
			if (!jitter_goal)
			{
				if (g_cfg.antihit.random_delay)
				{
					math::random_seed(HACKS->global_vars->tickcount);
					jitter_goal = math::random_int(0, g_cfg.antihit.jitter_delay);
				}
				else
					jitter_goal = g_cfg.antihit.jitter_delay;
			}

			if (jitter_ticks >= jitter_goal)
			{
				flip_jitter = !flip_jitter;
				jitter_ticks = 0;
				jitter_goal = 0;
